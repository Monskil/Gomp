...
func Handle_orders(new_order_bool_chan chan bool, new_order_chan chan Order, update_order_chan chan Order, external_order_list_chan chan [global.NUM_GLOBAL_ORDERS]Order, internal_order_list_chan chan [global.NUM_INTERNAL_ORDERS]Order) {
	...
	for {
		...
		case catch_update_order := <-update_order_chan:
			...
			Update_state(update_order, external_order_list_chan, internal_order_list_chan)
			if update_order.Order_state == Finished {
				fmt.Println("Order is finished")
				Delete_order(update_order, internal_order_list_chan, external_order_list_chan)
			}
...
func Update_state(update_order Order, external_order_list_chan chan [global.NUM_GLOBAL_ORDERS]Order, internal_order_list_chan chan [global.NUM_INTERNAL_ORDERS]Order) {
	fmt.Println("Updates state before")
	fmt.Println("My update_order: ", update_order)
	//var external_order_list [global.NUM_GLOBAL_ORDERS]Order
	if update_order.Button == global.BUTTON_UP || update_order.Button == global.BUTTON_DOWN {
		fmt.Println("Update external order loop. (guk)")
		//var external_order_list [global.NUM_GLOBAL_ORDERS]Order
		//external_order_list = <-external_order_list_chan
		external_order_list := <-external_order_list_chan
		fmt.Println("Pulled from external_order_list_chan.")
		external_order_list[1].Order_state = update_order.Order_state
		external_order_list_chan <- external_order_list
	} else {
		internal_order_list := <-internal_order_list_chan
		internal_order_list[1].Order_state = update_order.Order_state
...
/*
 func add_order(new_order Order, external_order_list [global.NUM_GLOBAL_ORDERS]Order, internal_order_list [global.NUM_INTERNAL_ORDERS]Order) [global.NUM_ORDERS]Order {
 	fmt.Println("Inside add_order function.")
 	if new_order.Button == global.BUTTON_UP || new_order.Button == global.BUTTON_DOWN {
 		fmt.Println("Going to add an external order.")
 	...
 	order_list := Make_my_order_list(internal_order_list, external_order_list)
 	return order_list
 }*/
